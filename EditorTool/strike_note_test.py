...
class MyTestCase(unittest.TestCase):
    ...
    def test_strike_note(self):
        print "************ strike note method ***********"
        ...
        #点删除线按扭
        self.driver.find_element_by_name('word strike').click()
        #点编辑区，进入编辑模式
        self.driver.find_element_by_class_name('TextView').click()

        #输入5行内容
        for i in range(1,6):
            self.driver.find_element_by_class_name('TextView').send_keys(editorconstant.strike[1])

        #finish
        self.driver.find_element_by_name('完成').click()
        #title
        self.driver.find_element_by_class_name('TextField').send_keys(editorconstant.strike[0])
        #back
        self.driver.find_element_by_name('返回').click()
        time.sleep(2)
        #检验 在列表上笔记创建成功
        self.assertTrue(self.driver.find_element_by_name(editorconstant.strike[0]).is_displayed(),'＊＊*删除线笔记创建失败！＊＊＊')
