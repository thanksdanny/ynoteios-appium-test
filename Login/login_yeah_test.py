...
class MyTestCase(unittest.TestCase):
    ...
    @unittest.skip()
    def test_loginyeah(self):
        print "************  login yeah user method ***********"
        #判断当前是否为 登录页
    ...
    def login_in(self):
        ...
        self.driver.find_element_by_class_name('TextField').send_keys(loginconstant.useryeah[0])
        self.driver.find_element_by_class_name('SecureTextField').send_keys(loginconstant.useryeah[1])

        self.driver.find_elements_by_name('登录')[2].click()
        time.sleep(3)
        #检验登录完成，跳转至了 最新 页
        self.assertTrue(self.driver.find_element_by_name('最新').is_displayed(),'＊＊＊＊yeah号：登录失败了！！＊＊＊')
        print '＊＊＊＊yeah号：登录成功！！＊＊＊'
