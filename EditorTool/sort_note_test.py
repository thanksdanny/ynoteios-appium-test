...
# 5再次打开笔记》进入编辑模式》检验
#检验：工具条上的sort元素的 value值为true（被选中）
...
class MyTestCase(unittest.TestCase):
    ...
    def test_sort_note(self):
        ...
        #edit
        #点 有序工具条
        self.driver.find_element_by_name('sort icon').click()
        #输入5行内容
        for i in range(1,6):
            self.driver.find_element_by_class_name('TextView').send_keys(editorconstant.sort[1])

        #finish
        self.driver.find_element_by_name('完成').click()
        #title
        self.driver.find_element_by_class_name('TextField').send_keys(editorconstant.sort[0])
        #back
        ...
        #check无序
        #再次打开此笔记
        self.driver.find_element_by_name(editorconstant.sort[0]).click()
        time.sleep(2)
        #点编辑区，进入编辑模式
        self.driver.find_element_by_class_name('TextView').click()
        time.sleep(2)
        #检验 sort的工具条元素的 value值为true
        self.assertTrue(self.driver.find_element_by_name('sort icon').get_attribute('value'),'＊＊有序中英文笔记创建失败！＊＊')
