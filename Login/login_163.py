...
#步骤：
# 1判断当前是否登录页
# 2若是，则直接输入帐号密码，点登录》打开登录页，输入帐号密码，点登录》检验
# 3若不是，先点 我的，点此登录》打开登录页，输入帐号密码，点登录》检验
#检验：当前界面为最新页
...
class MyTestCase(unittest.TestCase):
    ...
    @unittest.skip()
    def test_login163(self):
        print "************  login 163 user method ***********"
        #判断当前是否为 登录页
    ...
    def login_in(self):
        ...
        self.driver.find_element_by_class_name('TextField').send_keys(loginconstant.user163[0])
        self.driver.find_element_by_class_name('SecureTextField').send_keys(loginconstant.user163[1])

        self.driver.find_elements_by_name('登录')[2].click()
        time.sleep(3)
        #检验登录完成，跳转至了 最新 页
        self.assertTrue(self.driver.find_element_by_name('最新').is_displayed(),'＊＊＊＊163号：登录失败了！！＊＊＊')
        print '＊＊＊＊163号：登录成功！！＊＊＊'
