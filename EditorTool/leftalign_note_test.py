...
class MyTestCase(unittest.TestCase):
    ...
    def test_leftalign_note(self):
        print "************ leftalign note method ***********"
        ...
        #点 左对齐 按扭
        self.driver.find_element_by_name('leftalign').click()
        #点编辑区，进入编辑模式
        self.driver.find_element_by_class_name('TextView').click()

        #输入5行内容
        for i in range(1,6):
            self.driver.find_element_by_class_name('TextView').send_keys(editorconstant.leftalign[1])

        #finish
        self.driver.find_element_by_name('完成').click()
        #title
        self.driver.find_element_by_class_name('TextField').send_keys(editorconstant.leftalign[0])
        #back
        self.driver.find_element_by_name('返回').click()
        time.sleep(2)
        #检验 在列表上笔记创建成功
        self.assertTrue(self.driver.find_element_by_name(editorconstant.leftalign[0]).is_displayed(),'＊＊*左对齐笔记创建失败！＊＊＊')
