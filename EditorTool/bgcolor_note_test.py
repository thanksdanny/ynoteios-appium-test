...
class MyTestCase(unittest.TestCase):
    ...
    def test_bgcolor_note(self):
        print "************ bgcolor note method ***********"
        ...
        #点 背景色 按扭
        self.driver.find_element_by_name('word bgcolor').click()
        #点编辑区，进入编辑模式
        self.driver.find_element_by_class_name('TextView').click()

        #输入5行内容
        for i in range(1,6):
            self.driver.find_element_by_class_name('TextView').send_keys(editorconstant.bgcolor[1])

        #finish
        self.driver.find_element_by_name('完成').click()
        #title
        self.driver.find_element_by_class_name('TextField').send_keys(editorconstant.bgcolor[0])
        #back
        self.driver.find_element_by_name('返回').click()
        time.sleep(2)
        #检验 在列表上笔记创建成功
        self.assertTrue(self.driver.find_element_by_name(editorconstant.bgcolor[0]).is_displayed(),'＊＊*背景色笔记创建失败！＊＊＊')
